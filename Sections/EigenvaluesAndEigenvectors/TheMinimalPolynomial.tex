...
\end{proof}
    ...
    \item[25] Suppose $V$ is finite-dimensional, $T \in \mathcal L(V)$, and $U$ is a subspace of $V$ that is invariant under $T$.

    \begin{enumerate}
        \item[(a)] Prove that the minimal polynomial of $T$ is a polynomial multiple of the minimal polynomial of the quotient operator $T/U$.

        \begin{proof}
            Let's say that the minimal polynomial of $T$ is $p$ and the minimal polynomial of $T|_U$ is called $p_U$. Then $p(T|_U)$ clearly equals $0 + U$, so $p$ polynomial multiple of $p_U$. 
        \end{proof}
        \item[(b)] Prove that
        \[
            \text{(minimal polynomial of } T|_U\text{)} \times \text{(minimal polynomial of } T/U\text{)}
        \]
        is a polynomial multiple of the minimal polynomial of $T$.

        \begin{proof}
            We will prove that that polynomial sends all vectors to 0 when $T$ is plugged in. Suppose that we have $v \in V$. After applying the minimal polynomial of $T/U$, all that will be left is some $u \in U$, since $p_{T/U}(v + U) = 0 + U$. After applying the minimal polynomial of $T|_U$, we will be left with nothing. 
        \end{proof}
    \end{enumerate}
